...
## ADR-011：桌寵可拖曳 + 記住位置（覆蓋 Phase5 §7.1「不做位置記憶」）

- **狀態**：Accepted (2026-07-04) — 使用者拍板
- **Context**：使用者希望「按著介面可以拖曳畫面」——能自己把桌寵移到桌面想要的位置。原 Phase5 §7.1 把拖曳/位置記憶延後。
- **Decision**：**抓住角色（旅人）拖曳來移動整個桌寵視窗**；記住位置（UserDefaults）、下次開在原處。
  - **守非侵入**（ADR-002/Calm Tech）：只有「角色上」可抓拖（背景區維持點擊穿透、不擋桌面操作）。
  - **短按 vs 拖移**：以移動距離門檻區分——沒移動＝暖心回應（ADR-006 不變），移動超過門檻＝拖曳移動視窗（該次不觸發暖心）。
  - 多螢幕：拖後位置夾在有效螢幕內；螢幕變更時若記憶位置失效則回退合理落點。
- **Rationale**：SDT 自主（使用者決定同伴待在哪）；桌寵慣例。角色仍固定在視窗內左側（ADR-009 不變，那是「角色在場景內」，此為「視窗在桌面上」，兩者不衝突）。
- **Consequences**：Phase5 §7.1 的「不做位置記憶」被本 ADR 覆蓋；新增 UserDefaults 位置鍵（與偏好同層）。
