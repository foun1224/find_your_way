...
- **更新 (2026-07-05，使用者拍板)：改為主角獨行、移除同行旅伴（Superseded 同行部分）。**
  - **Decision**：不生成同行旅伴（CompanionNode），不觸發「相遇旅伴」story beat，companion 類相遇卡維持關閉（companionJoined 恆 false）。
  - **保留為休眠**：CompanionNode 類、char_companion 素材、companion 相遇卡與 meetDistance 邏輯**保留在碼庫但不啟用**（日後想加回容易，不刪死）。
  - **「有人」感如何維持**：世界各地域仍有**居民 NPC**（`RegionNpcScatter`，農夫/商人/國王…），社會臨場感（`02` §2）由「路過有人的世界」承載，而非固定同行者。母語敘事「我→我們」的 `14_STORYLINE` 同行弧線相應改為「獨行者路過眾人」——待使用者要時再重寫敘事。
  - **Rationale**：使用者偏好獨行的旅人美學。尊重使用者對核心體驗的選擇。
...
- **更新 (2026-07-04，使用者要求「背景也可以拖曳」)**：**整個視窗皆可抓拖**（不再限於角色）。
  - 取捨：整窗接收滑鼠事件 → 桌寵**footprint 不再點擊穿透**（會擋住其下桌面點擊）。因桌寵現為整片不透明場景（本就看不透）且使用者通常置於角落，代價可接受、亦為桌寵常態。**部分修正 ADR-002 的「非互動區點擊穿透」**：現在整個桌寵區都是互動區。
  - 點擊 vs 拖曳仍以移動門檻區分：**角色上**短按＝暖心回應（ADR-006 不變，需對游標做角色 hit-test）；**背景上**短按＝無作用（被消費、不穿透）；任一處拖移＝移動視窗。
  - 保留：位置記憶、螢幕夾取、`.accessory` 不搶焦點、floating 置頂。
  - 未來若使用者要回到「背景穿透、只有角色可拖」，另加偏好開關（暫不做）。
