...
## Phase 4 — 陪伴的質感 (Companionship Polish) 🔄 進行中（使用者提供像素素材）
> 美術來源：使用者 ChatGPT 產出 `design/assets/asset_sheet.png`（兩角色4向走路+道具+分層背景，像素風、側面橫向捲軸，ADR-009 確認）。規格 `12_PHASE4_SPEC.md`。分期 4a 背景+主角走路 / 4b 旅伴+道具 / 4c 晝夜+天氣 / 4d 微互動。
