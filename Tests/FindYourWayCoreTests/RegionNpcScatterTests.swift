...
final class RegionNpcScatterTests: XCTestCase {
    ...
    func testPastoralRegionsGetFarmerBakerChildMerchantMusician() {
        ...
        for region: RegionType in [.meadowOrigin, .village3] {
    ...
    /// 接手任務：聖光之城配置學者/藥師/樂師/商人（重用既有共享 `npc/` 角色，與浮空魔法之城
    /// 同款人口組成但獨立排法，收束天界奇幻序列）。
    func testHolyCityGetsScholarApothecaryMusicianMerchant() {
        let expectedNames: Set<String> = ["scholar", "apothecary", "musician", "merchant"]
        let slots = RegionNpcScatter.slots(for: .holyCity)
        XCTAssertEqual(Set(slots.map(\.npcName)), expectedNames)
        XCTAssertEqual(slots, RegionNpcScatter.holyCitySlots)
    }

    /// 接手任務：蒸氣龐克飛船城配置旅人/商人/學者/藥師（重用既有共享 `npc/` 角色，與雪山
    /// 王國同款人口組成但獨立排法，呼應飛船城作為工業貿易樞紐的往來人跡）。
    func testSteampunkCityGetsTravelerMerchantScholarApothecary() {
        let expectedNames: Set<String> = ["traveler", "merchant", "scholar", "apothecary"]
        let slots = RegionNpcScatter.slots(for: .steampunkCity)
        XCTAssertEqual(Set(slots.map(\.npcName)), expectedNames)
    ...
    func testEachConfiguredRegionHasThreeToFourSlots() {
        for region: RegionType in [.meadowOrigin, .village3, .seaCity, .harbor, .hotspringVillage, .snowMountain, .steampunkCity, .futureCity, .skyVillage, .mountainPalace, .magicCity, .holyCity, .skyCity] {
